...
<div class="emptyProfileInfo2">

<table style="color:#000;">
  <tr>
    <td><img src="jpg/compra.jpeg" width="150" height="128" /></td>
    <td colspan="3" align="center"><table width="235" border="1">
      <tr>
        <td colspan="3" align="center">Pedido</td>
      </tr>
      <tr>
        <td colspan="3" align="center">&nbsp;</td>
      </tr>
      <tr>
        <td width="86">Elementos: &nbsp;</td>
        <td colspan="3" align="right">
<?php
		$sql = "SELECT (COUNT(`facturas`.`id`)) as cantidad FROM `facturas` WHERE `id_venta`=".$_SESSION['venta'];
		include('php/consultar.php');
...
<?php
	$sql="SELECT (SUM(`facturas`.`valor_producto`)) as total FROM `facturas` Where `id_venta`=".$_SESSION['venta'];
	include('php/consultar.php');
...
?>
      ...
      <tr>
        <td>&nbsp;</td>
        <td width="103" align="right"><a href=""><img src="gif/goimage_ie.gif" width="23" height="22" alt="Comprar" /></a></td>
        <td width="24"><a href="php/validar_compra.php"><img src="gif/20x20_trash.gif" width="20" height="20" alt="Eliminar"/></a></td>
      </tr>
    </table></td>
  </tr>
  <tr>
    <td align="center" height="30" width="150">Producto</td>
    <td width="48" align="center">Valor</td>
    <td width="76" align="center">Cantidad</td>
    <td width="55">&nbsp;</td>
  </tr>
<?php
	$sql="SELECT (`facturas`.`id`) as id,(`productos`.`nombre`) as nombre, (`valor_producto`) as valor,(`cantidad_producto`) as cantidad FROM `facturas`,`productos` WHERE `facturas`.`id_producto` = `productos`.`id` AND  `facturas`.`id_venta`=".$_SESSION['venta'];
	$result =  @mysql_query($sql, $_SESSION['link']);

	if ($result) {
		while($row = @mysql_fetch_assoc($result)) {
			echo '<tr>';
			echo '<td height="30">'.$row['nombre'].'</td>';
			echo '<td align="right">$'.$row['valor'].'</td>';
			echo '<td align="center">'.$row['cantidad'].'</td>';
			echo '<td><a href="php/eliminar_producto.php?id='.$row['id'].'"><img src="gif/20x20_trash.gif" width="20" height="20" alt="Eliminar"/></a></td>';
			echo '</tr>';
		}
	}	
	mysql_free_result($result);
